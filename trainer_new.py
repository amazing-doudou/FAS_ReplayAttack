...
from datasets import init_dataloader_new,image_preprocessing
import torch.onnx
import random
...
class Model:
    def __init__(self, opt):
        ...
        self.model= init_model.get_model(opt.net_type,opt.pretrained)
        self.model = self.model.to(self.device)
        if opt.ngpu>0:
            self.model = nn.DataParallel(self.model)
        self.optimizer = utils.get_optimizer(self.model, self.opt)
        if os.path.exists(opt.log_dir):
            checkpoint = torch.load(opt.log_dir)
            print(checkpoint.keys())
            self.model.load_state_dict(checkpoint['model_state_dict'])
            self.optimizer.load_state_dict(checkpoint['optimizer'])
            self.epoch = checkpoint['epoch']
            print('加载 epoch {} 成功！'.format(self.epoch))
        else:
            self.epoch = 0
            print('无保存模型，将从头开始训练！')
        ...
        self.loss = init_model.init_loss(opt.loss_type)
        ...
        self.train_loader = datasets.generate_loader_new(opt,'train') 
        self.test_loader = datasets.generate_loader_new(opt,'val',True)
    ...
    def train_epoch(self):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            (ori_data,target),(crop_data,_) = data[0],data[1]
            ori_dataset = image_preprocessing.patchify(self.opt,ori_data)
            ...
            if self.opt.loss_type == 'bce':
                ...
                if np.random.rand() < self.opt.smooth_rate:
                    ...
                    smooth = 0
                self.smooth_label=utils.LabelSmoothingLoss(classes=3,smoothing=smooth, dim=-1)
                ...
                if np.random.rand() < self.opt.smooth_rate:
                    ...
                    loss_tensor = utils.FocalLoss(gamma=0.5,alpha= 0.0)(output,target)
    ...
    def test_epoch(self):
        ...
        print('test epoch'+str(self.epoch)+'_'+str(self.itr)+'\n'+'result is:'+'\ntpr_average:',tpr_aver,'\nfpr_average:',fpr_aver,accuracy_str) 
        result_msg='test epoch'+str(self.epoch)+'_'+str(self.itr)+' result is:'+'\ntpr_average:'+str(tpr_aver)+'\nfpr_average:'+str(fpr_aver)+accuracy_str+'\n'
    ...
    def log_epoch(self):
        ...
        out_test+='\nTest:  '
        tpr,fpr=self.test_metrics.get_accuracy_tpr_fpr()
        out_test += 'test epoch'+str(self.epoch)+'_'+str(self.itr)+' result is:'+'\ntpr_average:'+str(tpr)+'\nfpr_average:'+str(fpr)+'\n'
