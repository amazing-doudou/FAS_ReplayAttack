...
class Model:
    def __init__(self, opt):
        ...
        self.model= models.get_model_single(opt.net_type, 
                                            opt.classifier_type, 
                                            opt.pretrained,
                                            int(opt.nclasses))
    ...
    def train(self):
        ...
        print("img_size:",self.opt.img_size)
        print('model:',self.opt.net_type)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (rgb_data, target) in enumerate(self.train_loader):
          ...
          #  ir_data = ir_data.to(self.device)
            ...
            #print("rgb_data size is",rgb_data.shape)
    ...
    def save_checkpoint(self, filename):     # Save model to task_name/checkpoints/filename.pth
        ...
        #onnx_path = os.path.join(self.opt.out_path,'checkpoints', '1.onnx')
        #torch.save(self.state, fin_path)
        ##lxf change code
        
        
        self.model.eval()
        torch.save(self.model.state_dict(),fin_path)
        x=Variable(torch.randn(1, 3, self.opt.img_size, self.opt.img_size))
        print("x_shape:",x.shape)
        x = x.float()
        x=x.to(self.device)
        model=self.model
        #model.to(device)
        torch_out = torch.onnx.export(model,x,"./onnx_model/model_test_0113"+str(self.epoch)+".onnx")
        print("i have save the onnx is :",torch_out)
        #onnx_model=onnx.load("/mnt/sda1/nb_project/onnx_model/model_test0.onnx")
        #print("the onnx is:",onnx_model)
        ##change end
        if self.best_epoch:
